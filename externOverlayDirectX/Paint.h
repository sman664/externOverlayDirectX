...
const float VIRTUAL_SCREEN_WIDTH = 1200; //this is the screen width you develop/test your hack with, I use same for every game
const float GAME_UNIT_MAGIC = 600; //magic scaling number you adjust so the boxes fit the players in your game
...
public:	
	Paint();
	Paint(HANDLE hProc, HWND overlayHWND, HWND targetHWND, int width, int height);
	void Draw(HANDLE hProc);
	bool IsValidEnt(HANDLE hProc, uintptr_t ent);
